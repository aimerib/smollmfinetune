...
class DatasetManager:
    ...
    async def _paraphrase(self, text: str) -> str:
        ...
        try:
            response = await self.client.chat_complete(
                messages=[
                    {"role": "system", "content": "Paraphrase the following text to create a natural variation. Keep the same meaning but change the wording and sentence structure. Return only the paraphrased text."},
    ...
    def _generate_temporal_system_prompt(self, character: Dict[str, Any],
        ...
        base_prompt += " Respond naturally and stay in character."
    ...
    async def evaluate_nsfw_quality(self, response: str, character: Dict[str, Any], 
        ...
        return await content_evaluation.evaluate_nsfw_quality(self.client, response, character, prompt)
    ...
    def extract_intimate_speech_patterns(self, character: Dict[str, Any]) -> Dict[str, List[str]]:
        """Extract intimate speech patterns from character"""
        mes_example = character.get('mes_example', '')
        char_name = character.get('name', 'Assistant')
        return character_analysis.extract_intimate_speech_patterns(mes_example, char_name)
    ...
    async def generate_exploration_prompts(self, character: Dict[str, Any], num_prompts: int = 50) -> List[str]:
        """Generate diverse prompts exploring character traits"""
        return await prompt_generators.generate_exploration_prompts(self.client, character, num_prompts)
    ...
    def deduplicate_prompts(self, prompts: List[str], similarity_threshold: float = 0.8) -> List[str]:
        """Remove similar prompts from list"""
        return prompt_generators.deduplicate_prompts(prompts, similarity_threshold)

    async def generate_emotional_variations(self, base_prompt: str, character: Dict[str, Any]) -> List[str]:
        """Generate emotional variations of a prompt"""
        return await prompt_generators.generate_emotional_variations(self.client, base_prompt, character)

    async def enhance_prompt_with_context(self, prompt: str, character: Dict[str, Any]) -> str:
        """Enhance prompt with contextual information"""
        return await prompt_generators.enhance_prompt_with_context(self.client, prompt, character)

    async def generate_conversation_flows(self, character: Dict[str, Any], num_flows: int = 10) -> List[List[str]]:
        """Generate multi-turn conversation flows"""
        return await prompt_generators.generate_conversation_flows(self.client, character, num_flows)
    ...
    async def _extract_and_simplify_facts(self, character: Dict[str, Any], max_facts: int = 20) -> List[str]:
        ...
        return await factual_qa.extract_and_simplify_facts(self.client, character, max_facts)

    async def _generate_factual_qa_variations(self, fact: str, character: Dict[str, Any], 
                                            *, num_variations: int = 3, length_category: str = "short") -> List[Dict[str, str]]:
        """Generate factual Q&A variations"""
        return await factual_qa.generate_factual_qa_variations(self.client, fact, character, 
                                                              num_variations=num_variations, length_category=length_category)

    async def generate_factual_qa_dataset(self, character: Dict[str, Any], num_facts_to_use: int = 15, 
                                         variations_per_fact: int = 3, progress_callback: Optional[Callable] = None,
                                         stage_callback: Optional[Callable] = None) -> List[Dict[str, Any]]:
        """Generate factual Q&A dataset"""
        return await factual_qa.generate_factual_qa_dataset(
            self.client, character, num_facts_to_use, variations_per_fact, 
            progress_callback, stage_callback, self.save_dataset
        )
    ...
    async def suggest_user_questions(
        ...
        **sampling_kwargs
        ...
        logger.info(f"📊 Generated {len(results)} unique questions from {len(raw_outputs)} outputs")
        return results
    ...
    async def generate_dataset(self, character: Dict[str, Any], num_samples: int = 80,
        ...
        try:
            ...
            scenarios = await prompt_generators.generate_scenario_based_prompts(self.client, character, character_knowledge, num_scenarios=3)
            for scenario in scenarios[:2]:  # Only use 2 scenarios
                scenario_prompts = scenario.get('prompts', [])
                for prompt in scenario_prompts[:2]:  # Only 2 prompts per scenario
            ...
            while len(all_prompts) < new_samples_needed * 2:
                ...
                if prompt not in seen_user_prompts and prompt not in [p.get('prompt', p) if isinstance(p, dict) else p for p in all_prompts]:
            ...
            for prompt_item in selected_prompts:
                prompt_text = prompt_item.get('prompt', prompt_item) if isinstance(prompt_item, dict) else prompt_item
            ...
            for i, (messages, response) in enumerate(zip(batch_prompts, responses)):
                if response and response.strip():
                    ...
                    if self.generation_config.enable_real_time_filtering and self.quality_filter:
                        filtered_sample = self.quality_filter.filter_sample(sample)
                        if filtered_sample.get('quality_score', 0) >= self.generation_config.quality_threshold:
    ...
    async def _generate_premium_dataset(self, character: Dict[str, Any], num_samples: int,
        ...
        return await self.generate_dataset(
            character, num_samples, max_tokens, temperature, top_p,
            progress_callback, append_to_existing, custom_system_prompt,
            extra_quality, QualityLevel.ENHANCED, **sampling_kwargs
        )

    # Helper method for dataset generation  
    async def _generate_question_variations(self, base_question: str, character: Dict[str, Any], 
