...
## Acceptance Criteria
- [ ] Create new module `utils/evaluation/personality_metric.py`.
- [ ] Implement `calculate_personality_alignment(response: str, big_five_scores: Dict[str, float]) -> float`.
...
- [ ] The judge prompt must include a rubric explaining what high and low scores for each of the Big-Five traits look like in conversation.
- [ ] Integrate this new metric into the `TrainingQualityTracker` in `utils/metrics.py`.
- [ ] The score (`avg_personality_alignment`) is logged to WandB during training.
- [ ] Unit test mocks the LLM call and verifies that the score is correctly parsed and logged.
