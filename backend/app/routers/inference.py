...
# Import business logic from app/utils
...
try:
    ...
    logger.warning("Narrative engine not available, using fallback inference")
...
class InferenceResponse(BaseModel):
    ...
    def from_triple_head(cls, request: InferenceRequest, output: TripleHeadOutput, 
        ...
        return cls(
            ...
            tokens_generated=len(output.generation_text.split()),
            cache_hit=cache_hit
        )

# Global inference engine instance
class InferenceEngine:
    """Production inference engine integrated into unified backend"""
    
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        self.config = config or self._default_config()
        
        # Initialize training service (for fallback)
        self.training_service = AsyncTrainingService()
        ...
        self.model_cache_size = self.config["model_cache_size"]
    ...
    def _default_config(self) -> Dict[str, Any]:
        """Default configuration for inference engine"""
        return {
            "max_concurrent": 4,
            "max_queue_size": 100,
            "model_cache_size": 3,
            "cache_ttl": 300,  # 5 minutes
            "enable_triple_head": True,
    ...
    async def _load_model(self, character_id: str) -> Any:
        ...
        if self.config["enable_triple_head"] and NARRATIVE_ENGINE_AVAILABLE:
            ...
            # Load character-specific adapter if available
            adapter_path = f"adapter_cache/{character_id}"
            if hasattr(model, 'load_adapter'):
                try:
                    model.load_adapter(adapter_path)
                    logger.info(f"Loaded adapter for character: {character_id}")
                except Exception as e:
                    logger.warning(f"Failed to load adapter for {character_id}: {e}")
        else:
            # Use simple fallback model for testing
            model = {"type": "fallback", "character_id": character_id}
    ...
    async def _generate_standard(self, model: Any, request: InferenceRequest,
        ...
        response = f"Echo: {request.prompt} (from unified backend inference)"
        ...
        while True:
            try:
                ...
                self.health_status.update({
                    ...
                    "active_requests": self.config["max_concurrent"] - self.processing_semaphore._value,
...
# Router endpoints
@router.post("/inference/generate", response_model=InferenceResponse)
async def generate(request: InferenceRequest):
    """Generate a response from the character model with triple-head architecture"""
    return await engine.generate(request)

@router.get("/inference/health")
async def health():
    """Retrieve health information from the inference service"""
...
@router.get("/inference/queue/status")
async def queue_status():
    """Get the current status of the generation request queue"""
